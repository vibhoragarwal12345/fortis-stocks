...
function Card({
  ...
  size?: "default" | "sm"
  /**
   * default  -- 1px border, no shadow (the everyday card)
   * elevated -- soft diffuse shadow, no border (for hero/feature cards)
   * flat     -- no border, no shadow (lives on a surface that already defines it)
   */
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center rounded-b-xl border-t bg-secondary/40 px-6 py-4 group-data-[size=sm]/card:px-4 group-data-[size=sm]/card:py-3",
        className,
      )}
      {...props}
    />
